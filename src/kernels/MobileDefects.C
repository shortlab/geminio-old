    ...
    _gc(getUserObject<GroupConstant>("user_object"))
{
  ...
  _cur_size = GeminioUtils::getGroupNumber(cur_var_name.c_str());
...
MobileDefects::computeQpResidual()
{
  ...
  if(_cur_size>0)
  {
    ...
    for (int i = 1; i <= _number_v-cur_size; ++i)
    {
    ...
    // ii reaction loss(-)
    // guarantee the largest size doesn't exceed _number_v
    for (int i = 1; i <= _number_i - cur_size; ++i)
    {
      //printf("reaction %d (-): %d %d\n",_cur_size,_cur_size,-i);
      res_sum += (*_val_i_vars[i-1])[_qp]*_u[_qp]*_gc._absorb(-cur_size,-i);
    }
...
MobileDefects::computeQpJacobian()
{
  ...
  if(_cur_size>0){//v type
    ...
    //vv reaction loss(-)
    for(int i=1;i <= _number_v-cur_size;i++){//guarantee the largest size doesn't exceed _number_v
...
//(*_val_v_vars[cur_size-1])[_qp]
    ...
    //ii reaction loss(-)
    for(int i=1;i <= _number_i-cur_size;i++){//guarantee the largest size doesn't exceed _number_v
      jac_sum += (*_val_i_vars[i-1])[_qp]*_gc._absorb(-cur_size,-i);
    }
    if(cur_size*2<=_number_i)
