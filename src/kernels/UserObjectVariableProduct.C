...
UserObjectVariableProduct::UserObjectVariableProduct(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _coeff(getParam<Real>("coeff"))
{
  ...
  {
    case 0:
      groupa = GeminioUtils::getGroupNumber(cur_var_name.c_str());
      groupb = groupa;
      break;

    case 1:
      groupa = GeminioUtils::getGroupNumber(cur_var_name.c_str());
      groupb = GeminioUtils::getGroupNumber(vars_names[0].c_str());
      break;

    case 2:
      groupa = GeminioUtils::getGroupNumber(vars_names[0].c_str());
      groupb = GeminioUtils::getGroupNumber(vars_names[1].c_str());
      break;
...
UserObjectVariableProduct::computeQpResidual()
{
  ...
  switch (_v_vals.size())
  {
    ...
    case 2:
      ...
      res = (*_v_vals[1])[_qp] * (*_v_vals[0])[_qp];
      break;
...
{
  const Real gc = _gc._absorb(groupa, groupb);
  switch (_v_vals.size())
  {
    case 0:
