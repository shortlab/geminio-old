...
GroupConstant::setGroupScheme()
{
  ...
  if (_GroupScheme == "Uniform")
  {
    ...
    if (_Ng_v > 0)
    {
      ...
      if (_single_v_group < _Ng_v)
      {
        ...
        for (int i = single_v_group + interval; count <= _Ng_v;){
          // i <= _num_v;
          GroupScheme_v.push_back(i);
          count++;
          i += interval;
  ...
  else if (_GroupScheme == "Gaussian")
  {
    ...
    auto gauss = [](Real x, Real sigma){ return 1.0 / sigma / std::sqrt(2.0 * libMesh::pi) * std::exp(-x * x / (2.0 * sigma * sigma));};
...
GroupConstant::setGroupConstant()
{
  ...
  int total_mobile_v = _v_size.size();
  int total_mobile_i = _i_size.size();
...
GroupConstant::emit_gc(int cr_start, int cr_end)
{
  ...
  const auto species = cr_start > 0 ? MaterialParameters::Species::V : MaterialParameters::Species::I;
  // denote mobility
  int tagi = 0;
  Real sum = 0.0;
  Real counter = 0.0;
  ...
  for (int j = pos_start; j < pos_end; ++j)
  {
    ...
    // emission of vacancy from vacancy cluster, from function in IronProperty.C
    sum += _material->emit(j, 1, _T, species, species, tagi, 1);
    counter += 1.0;
  }
  ...
  else
  {
    ...
    for (int i = pos_ot_start; i < pos_ot_end; ++i)
    {
      ...
      for (int j = pos_cr_start; j < pos_cr_end; ++j)
      {
        ...
        counter += 1.0;
        // printf("absorb i %d with i %d: %lf\n",i,j,absorb(i,j,MaterialParameters::Species::I,MaterialParameters::Species::I,_T,tagi,tagj));
      }
    }
  }
...
//[cr_start,cr_end)
Real
GroupConstant::_diff(int groupid) const
{
  const auto it = _diff_array.find(groupid);
...
{
  // TODO: have the keys be unique by making sure groupid2 is always larger than groupid1
  // that way only one find is necessary!
  {
