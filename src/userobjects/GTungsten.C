...
Real GTungsten::energy(int s,std::string species, std::string Etype) const{//unit:eV
...
Real GTungsten::D_prefactor(int n, std::string species) const{
//ref: Microstructural evolution of irradiated tungsten: Ab initio parameterisation of an OKMC model
    Real D0 = 0.0;//um^2/s
    if (species == "V")
...
//size S1 and S2
Real GTungsten::absorb(int S1, int S2, std::string C1, std::string C2,Real T, int tag1, int tag2) const{
...
Real GTungsten::absorbVV(int S1, int S2, int flag, Real T) const{
    Real result = 0.0;
    switch(flag){
        case 1:
        {
...
Real GTungsten::absorbVI(int S1, int S2, int flag, Real T) const{
    Real result = 0.0;
    switch(flag){
        case 1:
        {
...
Real GTungsten::absorbII(int S1, int S2, int flag, Real T) const{
    Real result = 0.0;
    switch(flag){
        case 1:
        {
...
Real GTungsten::diff(int S1, std::string C1,Real T) const {
	return D_prefactor(S1,C1)*exp(-energy(S1,C1,"migration")/Boltz_const/T);
}//in um^2/s

Real GTungsten::emit(int S1, int S2, Real T, std::string C1, std::string C2, int tag1, int tag2) const{
...
Real GTungsten::disl_ksq(int S1, std::string C1, Real T, int tag) const {
