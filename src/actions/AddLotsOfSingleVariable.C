...
AddLotsOfSingleVariable::act()
{
  ...
  if (isParamValid("emit_vv") && custom == true)
    ...
    for (unsigned int i = 0; i < number_v; ++i){
  ...
  for (int cur_num = 2; cur_num <= number_v; cur_num++)
  {
    ...
    _problem->addKernel("SingleVariable", "SingleV_" + var_name_v, params);
    ...
    _problem->addKernel("SingleVariable", "SingleV_" + var_name_other + "_other", params_other);
  ...
  for (int cur_num = 2; cur_num <= number_i; cur_num++)
  {
    ...
    params.set<Real>("coeff") = emit_coef; // loss "+"
    _problem->addKernel("SingleVariable", "SingleV_" + var_name_i, params);
    ...
    emit_coef = -ii[cur_num-1];
    // emission coefficient from larger size to current size; gain should be negative in kernel
    params0.set<Real>("coeff") = emit_coef;
    _problem->addKernel("SingleVariable", "SingleV_" + var_name_i2 + "_1_" + Moose::stringify(cur_num), params0);
  ...
  if(isParamValid("emit_ii") && custom == true)
    ...
    for (unsigned int i = 0; i < number_i; ++i){
        ...
        for(int j=0;j<number_i;j++){
            ...
            Real temp_coef = emit(i+1,j+1,temp,"I","I",tagi,tagj);
