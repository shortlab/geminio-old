...
registerMooseAction("GeminioApp", AddGImmobile, "add_kernel");
...
AddGImmobile::act()
{
  ...
  for (unsigned int cur_size = num_mobile_v + 1; cur_size <= number_v; ++cur_size)
  {
    ...
    {
      coupled_i_vars.push_back(name() + "0i" + Moose::stringify(i));
      coupled_i_vars.push_back(name() + "1i" + Moose::stringify(i));
    }

    std::vector<VariableName> coupled_v_vars;
    for (unsigned int i = 1; i <= num_mobile_v; ++i)
    {
      coupled_v_vars.push_back(name() + "0v" + Moose::stringify(i));
      coupled_v_vars.push_back(name() + "1v" + Moose::stringify(i));
    }
    
    for (unsigned int i = num_mobile_v; i >= 1; --i)
    {
      // for vv reaction gain(+)
      coupled_v_vars.push_back(name() + "0v" + Moose::stringify(cur_size-i));
      coupled_v_vars.push_back(name() + "1v" + Moose::stringify(cur_size-i));
    }

    coupled_v_vars.push_back(name() + "0v" + Moose::stringify(cur_size));
    coupled_v_vars.push_back(name() + "1v" + Moose::stringify(cur_size));
    ...
    {
      coupled_v_vars.push_back(name() + "0v" + Moose::stringify(cur_size + 1));
      coupled_v_vars.push_back(name() + "1v" + Moose::stringify(cur_size + 1));
    }

    std::string var_name_v;
    
    var_name_v = name() +"0v"+ Moose::stringify(cur_size);
    ...
    _problem->addKernel("GImmobileL0", "GImmobileL0_" + var_name_v + "_" + Moose::stringify(cur_size), params);
    //printf("add GImmobileL0: %s \n",var_name_v.c_str());

    var_name_v = name() +"1v"+ Moose::stringify(cur_size);
    ...
    params1.set<UserObjectName>("user_object") = group_constant_name;
    ...
    _problem->addKernel("GImmobileL1", "GImmobileL1_" + var_name_v + "_" + Moose::stringify(cur_size), params1);
  ...
  for (unsigned int cur_size=num_mobile_i+1; cur_size<=number_i; cur_size++)
  {
    ...
    for (unsigned int i = num_mobile_i; i >= 1; --i){
      ...
      coupled_i_vars.push_back(name() + "0i" + Moose::stringify(cur_size-i));
      coupled_i_vars.push_back(name() + "1i" + Moose::stringify(cur_size-i));
    }

    coupled_i_vars.push_back(name() + "0i" + Moose::stringify(cur_size));
    coupled_i_vars.push_back(name() + "1i" + Moose::stringify(cur_size));
    ...
    {
      coupled_i_vars.push_back(name() + "0i" + Moose::stringify(cur_size+1));
      coupled_i_vars.push_back(name() + "1i" + Moose::stringify(cur_size+1));
    }
    
    std::string var_name_i;

    var_name_i = name() + "0i" + Moose::stringify(cur_size);
    InputParameters params = _factory.getValidParams("GImmobileL0");
